...
- **Verify the isolation held before you report the batch.** After the run, check that no worker databases leaked: `psql -lqt | cut -d\| -f1 | grep -E '<app>_test_i[0-9]+_[0-9]+'` should return nothing. A hit means some command ran without `PARALLEL_WORKERS=1` — the wrapper was bypassed somewhere. Say so in the report rather than cleaning up quietly; that is the signal the mechanism is leaking. (Observed 2026-07-27: `…_test_i367_0` through `_i367_11`, twelve worker DBs from a single missed prefix.)
