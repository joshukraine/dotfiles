...
description: Fan out a batch of autopilot-queued issues to parallel background worktree subagents — each runs /autopilot — with a gating review one model tier above every build (Opus reviews Sonnet builds, Fable reviews Opus builds).
...
Run the vetted `autopilot-queued` queue as a parallel batch: one background `isolation: worktree` subagent per issue, each running `/autopilot <n>` end-to-end, with a gating `/code-review` one model tier above every build (Opus for Sonnet-built PRs, Fable for Opus-built PRs). This is the **run** half of the triage → run split; `/autopilot-triage` is the **vet** half that fills the queue.
...
## The build-model policy — review one tier above the build

Each issue's _build_ subagent runs at a model chosen for the issue's class; the _gating review_ always runs **one model tier above the build** (ladder: Sonnet 5 → Opus 4.8 → Fable). Assign the build model per issue by reading its labels and scope:
...
- Build the run plan: for each issue, its assigned build model (+ a one-line why) and its tier (`pr`, or `merge` if named in `--merge` — and remember a `--merge` issue is forced to a Fable build).
...
- `model:` the issue's assigned build model (Sonnet or Opus per the policy above; Fable for any `--to merge` issue).
...
- Kick off its tier-above gating review (Step 4) right away — don't wait for the whole batch: an **Opus** review for a Sonnet build, a **Fable** review for an Opus build. (A `--to merge` issue was Fable-built and Fable-reviewed inside its own `/autopilot` run — no orchestrator review needed.)
...
For each review-ready PR, spawn a review subagent one model tier above its build — **Opus** for a Sonnet-built PR, **Fable** for an Opus-built PR:

- `model: opus` (Sonnet build) or `model: fable` (Opus build), `isolation: worktree`.
...
| ----- | -- | ----- | ------ | ------- |
| #847  | #M | Fable  | (internal)      | merged + deployed ✓ |
| #851  | #P | Sonnet | clean (Opus)    | PR-ready |
| #863  | #Q | Opus   | 2 fixes (Fable) | PR-ready |
