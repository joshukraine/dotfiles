...
- **Launch-status callout (lifecycle-gated):** for the default pre-review scratch copy, **omit** the entire `.callout` block. For the `--publish` (post-merge) version, determine the project's launch status from its `CLAUDE.md` § "QA Testing Policy" (the `Launch status:` flag), then include the matching `.callout`:
  - **Pre-launch**, or no QA Testing Policy is declared → the **production-verification** callout (testers without a local environment may verify on the live site; its data is disposable). This is the template's default `.callout`.
  - **Post-launch** → the **local-dev-only** callout instead: a warning that QA must run on a local dev checkout and must **not** be run against the live site, which holds real data. Emit no "verify on production" text. (See the template's comment for both snippets.)
...
5. **Render the HTML** following **Rendering the HTML artifact** above. This is the post-merge version, so **include the launch-status callout** — the production-verification callout when the project is pre-launch, the local-dev-only callout when it is post-launch. Save to `tmp/pr-<N>-walkthrough-published.html`.
