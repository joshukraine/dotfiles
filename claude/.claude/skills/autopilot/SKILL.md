...
- **Session model is _below_ the labeled tier** (e.g. Sonnet running a `model: opus` issue) — **stop and ask.** This is an under-build, the direction that produces quietly wrong work, and the announce is the cheapest possible moment to catch it. Report the mismatch and recommend re-invoking at the labeled tier.
