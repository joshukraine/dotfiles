...
| **SUPERSEDED** | Replaced by a later, deliberate ruling from Joshua — the line was doing its job, the job changed |

---

## Working Relationship

### The anti-terseness clause — `SUPERSEDED`

> Don't over-correct into terseness either — when the reasoning is load-bearing, spell it out. The discriminator: **explain the decision, not the syntax.**

Removed 2026-09-21. This was **added** on 2026-08-01 as part of Phase 7, as a guard against reading "retire tutorial mode" as "be curt." Seven weeks of live use showed it pulling the other way: paired with a "give him the _why_" instruction that carried no length budget, it read as standing permission to expand, and Joshua reported replies long enough that he had to work to digest them mid-session.

Replaced by three bullets — default to the short version, name things plainly, put the long version in memory/Obsidian/issues — which set the register positively instead of fencing off one failure mode. The discriminator sentence was not lost; "the decision and its trade-offs, never the syntax" folds it into the first bullet.

This is the first entry quarantined **after** Phase 1, and the first that is not an ablation cut at all. Logged in [`stumble-log.md`](stumble-log.md) under 2026-09-21.
...
### Step 1 (Get issue details) — `GENERIC`, **partially reverted**
...
> ⚠️ **This cut was wrong in one respect and was reverted the same day.** See the first row of [`stumble-log.md`](stumble-log.md).
>
> "…and **comments**" was load-bearing, because `gh issue view N` prints the body but **not** the comment thread. The original was already self-contradictory — an incomplete command paired with an instruction to extract something it does not return — and dropping the word "comments" removed the only prompt to go get them.
>
> Verified on `comix_distro` #512: bare output ends at the body; `--comments` prints _only_ the comments and omits the body, so it is not a drop-in replacement; the bare header does carry a `comments: N` count, but in a metadata block that is easy to skim past. The fix is a two-call fetch with the rationale inline, not a restoration of the original bullet.

This is the initiative's first real course-correction, and it sharpened the taxonomy — see **D7** on #252. A skill that hands over an _incomplete command_ is not the same as a skill that fails to correct model behavior. The former introduces the gap; only the latter is what ablation is meant to remove.
